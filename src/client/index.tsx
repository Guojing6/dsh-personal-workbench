...
/* ---- Apple-style visual refresh: balanced panes, translucent surfaces, softer hierarchy ---- */
.wb-app {
  --wb-accent: var(--dsw-alias-state-business-primary, #0a84ff);
  --wb-danger: #ff453a;
  --wb-warning: #ff9f0a;
  --wb-success: #30d158;
  --wb-surface: color-mix(in srgb, var(--dsw-alias-bg-layer-1, #1c1c1e) 86%, transparent);
  --wb-surface-strong: color-mix(in srgb, var(--dsw-alias-bg-layer-2, #2c2c2e) 92%, transparent);
  --wb-glass: color-mix(in srgb, var(--dsw-alias-bg-layer-1, #1c1c1e) 76%, transparent);
  --wb-border: color-mix(in srgb, var(--dsw-alias-label-primary, #f5f5f7) 18%, transparent);
  --wb-border-soft: color-mix(in srgb, var(--dsw-alias-label-primary, #f5f5f7) 10%, transparent);
  --wb-shadow-soft: 0 8px 28px rgba(0,0,0,.16);
  --wb-shadow-float: 0 18px 48px rgba(0,0,0,.28);
  background:
    linear-gradient(180deg, color-mix(in srgb, var(--wb-accent) 7%, transparent), transparent 210px),
    var(--dsw-alias-bg-base, #111);
}
.wb-h {
  gap: 10px;
  min-height: 62px;
...
.wb-title { font-size: 15px; font-weight: 650; letter-spacing: 0; }
.wb-title svg { color: var(--wb-accent); }
.wb-segmented {
  padding: 2px;
  border-radius: 9px;
...
.wb-seg.on {
  ...
  box-shadow: 0 1px 5px rgba(0,0,0,.18), inset 0 0 0 1px var(--wb-border-soft);
}
.wb-btn {
  min-height: 32px;
  border-radius: 8px;
...
.wb-btn:hover:not(:disabled) {
  ...
  border-color: color-mix(in srgb, var(--wb-accent) 38%, transparent);
  box-shadow: 0 6px 16px rgba(0,0,0,.12);
}
.wb-btn.primary {
  background: color-mix(in srgb, var(--wb-accent) 18%, transparent);
  border-color: color-mix(in srgb, var(--wb-accent) 42%, transparent);
}
.wb-pending-pill {
  min-height: 32px;
  border-radius: 999px;
  border-color: color-mix(in srgb, var(--wb-warning) 34%, transparent);
  background: color-mix(in srgb, var(--wb-warning) 14%, transparent);
  box-shadow: inset 0 0 0 1px color-mix(in srgb, #fff 5%, transparent);
}
.wb-body {
  background: color-mix(in srgb, var(--dsw-alias-bg-base, #111) 94%, var(--wb-accent) 6%);
}
.wb-nav {
  flex: 0 1 clamp(380px, 44%, 760px);
  min-width: 360px;
  padding: 0 18px 18px;
  border-right-color: var(--wb-border-soft) !important;
}
.wb-detail {
  flex: 1 1 56%;
  padding: 18px 20px;
...
.wb-row-meta { grid-template-columns: 68px 46px 56px 88px; }
.wb-day,
.wb-mday {
  border-radius: 8px;
  background: var(--wb-surface);
}
.wb-day { min-height: 96px; }
.wb-mday { min-height: 54px; }
.wb-day.today,
.wb-mday.today {
  border-color: color-mix(in srgb, var(--wb-accent) 48%, transparent) !important;
  background: color-mix(in srgb, var(--wb-accent) 11%, var(--wb-surface)) !important;
}
.wb-form {
  border-radius: 8px;
  gap: 11px;
...
.wb-form textarea,
.wb-session-search,
.wb-session-role-select,
.wb-quick-textarea {
  border-radius: 8px;
  background: color-mix(in srgb, var(--dsw-alias-bg-base, #111) 82%, transparent);
  border-color: var(--wb-border-soft);
}
.wb-modal-mask {
...
.wb-plan {
  ...
  background: color-mix(in srgb, var(--wb-accent) 7%, var(--wb-surface));
}
.wb-detail-actions {
  border-radius: 8px;
  background: var(--wb-glass);
...
.wb-detail-tabs { gap: 6px; }
.wb-detail-tab {
  border-radius: 8px;
  padding: 7px 12px;
}
.wb-detail-tab.on {
  background: color-mix(in srgb, var(--wb-accent) 11%, transparent);
  box-shadow: inset 0 0 0 1px color-mix(in srgb, var(--wb-accent) 22%, transparent);
}
.wb-quick-image-item,
.wb-send-button,
.wb-model-option,
...
const quickDocumentToContext = async (document: QuickDocumentDraft): Promise<string> => {
  const res = await api<{ content: string; name: string; truncated: boolean }>(
...
function WorkbenchApp({ runtime, closePanel }: { runtime: WorkbenchRuntime; closePanel: () => void }): JSX.Element {
  ...
  useEffect(() => { quickAttachmentsRef.current = quickAttachments }, [quickAttachments])
  useEffect(() => () => {
    for (const attachment of quickAttachmentsRef.current) {
      if (isQuickImageDraft(attachment)) URL.revokeObjectURL(attachment.previewUrl)
    }
  }, [])
  const addQuickAttachments = useCallback((files: FileList | readonly File[]): void => {
    const incoming = Array.from(files)
    const rejected = incoming.filter((file) => !isQuickAttachmentFile(file))
    const accepted = incoming.filter(isQuickAttachmentFile)
    if (rejected.length > 0) setError('快速录入仅支持图片、PDF 和 DOCX 文档')
    if (accepted.length === 0) return
    const remaining = Math.max(0, 10 - quickAttachmentsRef.current.length)
    const nextFiles = accepted.slice(0, remaining)
    if (nextFiles.length < accepted.length) setError('快速录入最多添加 10 个附件')
    if (nextFiles.length === 0) return
    setQuickAttachments((prev) => [
      ...prev,
      ...nextFiles.map((file) => isQuickImageFile(file)
        ? { id: createClientId(), file, previewUrl: URL.createObjectURL(file) }
        : { id: createClientId(), file }),
    ])
  }, [])
  const removeQuickAttachment = useCallback((id: string): void => {
    setQuickAttachments((prev) => {
      const attachment = prev.find((item) => item.id === id)
      if (attachment !== undefined && isQuickImageDraft(attachment)) URL.revokeObjectURL(attachment.previewUrl)
      return prev.filter((item) => item.id !== id)
    })
  }, [])
  const clearQuickAttachments = useCallback((): void => {
    setQuickAttachments((prev) => {
            ...
            <div className="wb-form-panel">
              ...
              >
                <textarea
                  ...
                  placeholder="一句话描述任务，例如：周五10:30接待重要客户；也可以粘贴或拖入图片、PDF 或 DOCX"
                />
